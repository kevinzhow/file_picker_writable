...
interface ActivityProvider : CoroutineScope {
  ...
  @MainThread fun openFile(fileInfo: Map<String, String>)
  @MainThread fun handleOpenUri(uri: Uri)
}
...
class FilePickerWritableImpl(private val plugin: ActivityProvider) :
  ...
  fun openFilePicker(result: MethodChannel.Result, allowedExtensions: String) {
    ...
    try {
      ...
      result.error("FilePickerNotAvailable", "Unable to start file picker, $e", null)
    }
  }
  ...
  override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?): Boolean {
    if (!arrayOf(REQUEST_CODE_OPEN_FILE, REQUEST_CODE_CREATE_FILE).contains(requestCode)) {
    ...
    if (resultCode == Activity.RESULT_CANCELED) {
      ...
      result.error("InvalidResult", "Got invalid result $resultCode", null)
      return true
    }
    plugin.launch {
      try {
  ...
  @MainThread
  private suspend fun handleFileUriResponse(result: MethodChannel.Result, fileUri: Uri) {
  ...
  @MainThread
  private suspend fun copyContentUriAndReturn(result: MethodChannel.Result, fileUri: Uri) {

    result.success(copyContentUriAndReturnFileInfo(fileUri))
  }
  ...
  override fun onNewIntent(intent: Intent): Boolean {
    ...
    //      plugin.logDebug("Not handling url $data (no supported scheme
    // $CONTENT_PROVIDER_SCHEMES)")
    //      return false
    //    }
    plugin.launch {
      try {
  ...
  suspend fun init() {
    isInitialized = true
    initOpenUrl?.let { uri -> handleUri(uri) }
    initOpenUrl = null
  }
